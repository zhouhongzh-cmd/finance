...
- 期现溢价阈值已支持按 `外盘指数` 与 `加密资产 × 合约桶` 配置，并单独写入 `config/premium_thresholds.json`
...
| 6 | 恒生指数期货、纳指/标普远月等外盘指数期货源待探索 | 待定 | P2 | 当前只把已验证可形成现货/期货对的数据入库 |
...
| 期现溢价 | `DONE` | 已接入外盘指数 + Top10 合约加密资产池 fetcher/strategy、GUI 阈值、独立时钟与快照表，并补齐 Top10 快照回读与合约桶展示 |
