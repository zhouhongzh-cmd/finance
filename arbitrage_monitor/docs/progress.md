...
- 已接入 `BTC + A50` 期现溢价模块，支持独立调度、SQLite 快照、参数页和看板
...
- 期现溢价阈值已支持按 `BTC / A50` 分资产配置，并单独写入 `config/premium_thresholds.json`
...
- BTC / A50 live 取数仍依赖 `yfinance` 与 `akshare` 可用性，当前通过多级取价和按合约跳过保证连续运行
...
| 期现溢价 | `DONE` | 已接入 BTC + A50 fetcher/strategy、GUI 阈值、独立时钟与快照表 |
