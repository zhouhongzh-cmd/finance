...
- **⚠️ 当前观察**:
  - 这轮清理范围只覆盖当前已存在的历史报警与保证金快照
  - 若后续增加更多 snapshot 表，仍需把它们纳入同一套保留期治理
...
- **⚠️ 当前观察**:
  - 这轮已显著改善“伪隔离”问题，但价格类任务仍共用部分运行时资源
  - SQLite 写锁已降低竞争风险，但长期高负载下仍建议升级为单写队列
...
- **⚠️ 当前观察**:
  - 看板性能瓶颈已明显缓解，但 live 页面本质上仍依赖直接联网抓取
  - 若后续继续提速，应把看板主读取路径逐步切到数据库快照，而不是每次直接抓 live
...
- **⚠️ 当前仍未闭环**:
  - 当时通知队列框架仅完成基础设计；该限制已在后续迭代中收敛
- **⏭️ 下一步**:
  - 若正式推进通知能力，应先补需求，再补实现与测试
...
- **⚠️ 当前观察**:
  - 东方财富指数接口在当前环境中仍可能因代理/连接问题失败
  - 现阶段已通过“东方财富主源 + 新浪现货备源 + 中金财富保证金备源”打通期指实时链路
...
- **⚠️ 当前观察**:
  - 官方规则页作为口径源可信，但程序化访问可用性一般
  - 现阶段已通过“官方抓取优先 + 期货公司日度公告备源 + 默认最低保证金比例兜底”保证任务可稳定落地
...
- **⚠️ 研发结论**:
  - 东方财富和同花顺可以补齐绝大多数字段
  - 但个别字段口径仍难与集思录做到完全逐值一致，尤其是 `到期税前收益` 的计算口径
...
- **⏭️ 下一步**:
  - 飞书通知 `_send_feishu` 需实现真正的 HTTP POST 请求（用户明确表示暂不处理）
  - 企业微信 `_send_wecom` 待实现
  - DB 连接池优化（当前 SQLite 每次创建/关闭影响极小，可后续考虑）
